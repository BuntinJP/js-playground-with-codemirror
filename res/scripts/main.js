...
function executeCode() {
    ...
    if (code.trim() == '') {
        codeOutput.innerHTML =
            '<h4 style="color: #ff5555">🤣 First write some code, then run it!</h4>';
        return;
    }
    try {
        ...
        codeOutput.innerHTML +=
            '</br><h4 style="color: #55ff55">🚀 Execution took ' +
            totalTime +
            'ms</h4>';
        codeOutput.innerHTML +=
            '<h4 style="color: #55ff55">💻 Code Returned: <span style="color: #f0f0f0">' +
            returnValue +
            '</span></h4>';
        ...
        codeOutput.innerHTML +=
            '<h4 style="color: #ff5555">Problem at line number: <span style="color: #f0f0f0">' +
            e.lineNumber +
            '</span></h4>';
    }
}
...
function initDrag(e) {
    startX = e.clientX;
    startWidth = parseInt(
        document.defaultView.getComputedStyle(codeOutput).width,
        10
    );
    ...
    vex.dialog.prompt({
        ...
        callback: function (value) {
            if (!value) {
                ...
                vex.dialog.alert(
                    'You cannot use special characters in file name!'
                );
                return;
            }
    ...
    if ((files = localStorage.getItem('files'))) {
        ...
        files.forEach((file) => {
            ...
            fileItem.addEventListener('click', (e) => {
                ...
                const textContent =
                    localStorage.getItem(currentFile) || '// write some code';
            ...
            deleteBtn.addEventListener('click', () => {
                vex.dialog.confirm({
                    message:
                        ...
                        fileItem.getAttribute('data-file') +
                        '?',
                    callback: function (value) {
                        if (value) {
